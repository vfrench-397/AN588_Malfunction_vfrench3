---
title: "mrubiog2000_PeerCommentary_vfrench3"
...
upper <- p1 + qnorm(1 - (1 - conf.level)/2) * sqrt(p1 * (1-p1)/(n1) + p2 * (1-p2)/n2)
ci <- c(lower, upper)
...
#Miguel: Awesome job! I really liked your function: it was easy to follow, to understand and I have learnt many things from it. I also felt you have a great domain of the statistical concepts you are using. I specially liked the "cat" function you used to show all the results at once and how you structure your code with spaces and comments.
#I appreciate that you wrote some examples at the bottom so one can check the function in action. Maybe, you can add one extra line in which the rule of thumb is not big enough to check how does it work too.
...
You can try to compute the regression manually but it does become difficult if the data has NAs. 
```{r}
y.data <- data[is.na(data$MaxLongevity_m) == FALSE & is.na(data$Brain_Size_Species_Mean) == FALSE, ] #First remove any rows where brain size or longevity posess NAs.
...
#Miguel: Good job at removing the NA data! (I tried to use is.na function too but I was unable to use it right). I liked how you set your variables for the next questions. Once more, everything in your code is clear and understandable.
...
#Miguel: I like a lot how you have set a function to provide the equation. This way you can calculate it for any data you have. This is really good!
...
#Miguel: The sequence of your coding is really good and easy to follow. Each step was well structured and explained.
...
Looking at the two models the log transformation seemed to create a better fitting model. The data wasn't as skewed to the left as the original data and the CIs were smaller and more consistent. This resulted in a higher R squared value indicating a stronger linear relationship between the variables than the original model suggests. 


#Miguel: I am not really sure why do we need to do the "log" analysis, but it resulted in a better model. Once again, the structure of the code was really good. I have no useful suggestions but keep up the good work.
...
#Overall commentary. I think this code is amazing: Structure, explanations, use of different functions and resources, etc was impressive. Congratulations! I learnt a lot of things and thanks to your coding I will probably understand better the statistical concepts than when just reading the modules.

#I am so sorry I could not give any useful piece of advice :( but this homework was overwhelming for me (plus I am still struggling with the concepts).